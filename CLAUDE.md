...
5. **R6 Tracker (`/stats`) — Spielerstatistiken über R6Data:**
   ...
   * **Eine Suche = 4 R6Data-Aufrufe** (`fullStats`, `operatorStats`, `seasonalStats`, `isBanned`) **× 2**: 5 s nach jeder erfolgreichen Suche lädt der Client das Profil einmal **still im Hintergrund nach** und tauscht die Anzeige ohne Flackern aus — R6Datas erster Abruf liefert oft noch deren alten Cache, erst der zweite die frischen Matches. Eine neue Suche bricht den ausstehenden Refresh ab. API hat ein Kontingent (Endpoint `/api/me/usage`); wird es knapp, den Auto-Refresh in `StatsPageClient.runSearch` drosseln.
