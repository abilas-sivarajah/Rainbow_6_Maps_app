...
5. **R6 Tracker (`/stats`) — Spielerstatistiken über R6Data:**
   ...
   * **Eine Suche = 4 R6Data-Aufrufe** (`fullStats`, `operatorStats`, `seasonalStats`, `isBanned`), plus stille Nachfass-Abrufe (s.u.). Wichtig: Die R6Data-**API liefert immer nur deren Cache**; die Suche selbst stößt R6Datas Aktualisierung aus Ubisoft an, die aber **~30–60 s dauert** (Nutzertest 19.07.2026: erneute Suche nach ~1 Min war frisch — ganz ohne r6data.com-Besuch). Deshalb fasst der Client nach jeder Suche **still bei 6 s / 15 s / 35 s / 70 s nach** und stoppt, sobald neuere Daten ankommen (Vergleich über den neuesten `rankHistory`-Zeitstempel); solange läuft die Kette, zeigt die Datenstand-Zeile „⏳ aktualisiere…". Profiltausch ohne Flackern, neue Suche bricht die Kette ab. Worst Case = 20 R6Data-Calls pro Suche (Kontingent: `/api/me/usage`) — wird es knapp, Versuche in `StatsPageClient.runSearch` reduzieren. Sackgassen, NICHT wieder probieren: unsichtbares iframe der r6data.com-Profilseite (blockt `X-Frame-Options: sameorigin`); der serverseitige Seiten-Ping `triggerR6DataRefresh` läuft weiter (best effort, Wirkung unklar).
